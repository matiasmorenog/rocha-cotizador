...
export async function GET(req: NextRequest) {
  ...
  const [hasModule, items, count] = await Promise.all([
    customerHasModule(customerId, "CONSUMABLES"),
    entryOnly
      ? Promise.resolve([])
...
export async function POST(req: NextRequest) {
  ...
  const customerId = session.user.customerId;
  if (!(await customerHasModule(customerId, "CONSUMABLES"))) {
    return NextResponse.json({ error: "Módulo no habilitado" }, { status: 403 });
  }
  ...
  const validItems = await db.stockItem.findMany({
    where: {
      id: { in: itemIds },
      ...stockItemWhereForModule("CONSUMABLES"),
    },
    select: { id: true },
  });
  ...
  const row = await db.$transaction(async (tx) => {
    ...
    if (existing) {
      await tx.consumableCountLine.deleteMany({
        where: { countId: existing.id },
      });
